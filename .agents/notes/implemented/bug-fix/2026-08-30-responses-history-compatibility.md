...
Responses identity and durable history now follow an immutable parent-linked graph. Each request allocates its public response ID before upstream execution; a root uses that ID as its request-session identity, while a continuation reads signatures under `previous_response_id` and writes new state under its child response ID. A stored node contains only its input delta, response output and inherited request metadata. Materialization walks root to leaf, so deleting a parent's lookup key does not invalidate an already stored child. Stream completion is persisted before the terminal event becomes observable. Message item IDs use a separate random prefix and therefore do not alias the public response ID.

Continuation deltas deliberately implement only the target-era rules: compaction starts a new root, an exact stored prefix contributes only its suffix, and otherwise the last shared non-empty item ID defines the suffix boundary. Inputs with no such boundary remain entirely new. Later semantic suffix matching and routing-lineage redesigns are not part of this decision.
...
Identity and branching regressions additionally cover root and continuation signature keys, caller-owned response IDs on every SSE lifecycle event, exact-prefix replay, ID-boundary deltas, sibling branches, parent-key deletion, durable restart from an embedded ancestor, compaction roots, and rejection of later semantic suffix heuristics. The streaming mapper test pins the independent message-item ID prefix.
