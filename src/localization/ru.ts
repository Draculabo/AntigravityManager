const ru = {
  ...
  proxy: {
    ...
    config: {
      ...
      'allow-local-video-paths': 'Разрешить локальные пути к видео',
      'allow-local-video-paths-desc':
        'Разрешает API-запросам читать через video_url локальные файлы, доступные вашей учетной записи. Включайте только для доверенных клиентов прокси.',
      'global-system-prompt-title': 'Глобальная системная инструкция',
      'global-system-prompt-description':
        'Добавляет эту инструкцию перед системными инструкциями клиента в каждом запросе к прокси.',
      'global-system-prompt-placeholder':
        'Введите глобальную системную инструкцию...\nНапример: отвечайте на упрощённом китайском и кратко объясняйте изменения кода.',
      'global-system-prompt-character-count': '{{count}} символов',
      'global-system-prompt-long-warning':
        'Инструкция длиннее 2 000 символов может заметно сократить доступное окно контекста.',
