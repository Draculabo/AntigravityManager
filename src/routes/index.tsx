...
import { listAccounts, addAccountSnapshot, switchAccount, deleteAccount } from '@/actions/account';
  ...
  if (isErrorAccounts || isErrorCurrent) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 p-6 text-center">
        <div className="text-destructive text-lg font-semibold">{t('error.generic')}</div>
        <p className="text-muted-foreground text-sm">
          {errorAccounts?.message || errorCurrent?.message}
        </p>
        <Button onClick={() => queryClient.invalidateQueries()}>{t('action.retry')}</Button>
      </div>
    );
  }
      ...
      <div className="mb-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {accounts?.map((account) => (
          <AccountCard
            ...
            isSwitching={switchMutation.isPending && switchMutation.variables === account.id}
            isDeleting={deleteMutation.isPending && deleteMutation.variables === account.id}
          />
        ))}
        ...
        {accounts?.length === 0 && (
          <div className="bg-muted/10 col-span-full rounded-lg border-2 border-dashed py-12 text-center">
            ...
            <p className="text-muted-foreground mt-1 mb-4">{t('home.noBackups.description')}</p>
            <Button
              variant="secondary"
