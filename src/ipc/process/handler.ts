...
export async function isProcessRunning(): Promise<boolean> {
  try {
    ...
    if (platform === "win32" || isWsl()) {
      // Use tasklist.exe for both Windows and WSL
      // In WSL, use absolute path to be safe
  ...
  try {
    ...
    if (platform === "darwin") {
      ...
      try {
        ...
        logger.warn("Graceful taskkill failed, proceeding to next stage");
      }
    }
        ...
        for (const line of lines) {
          ...
          if (platform === "win32") {
             ...
             if (line.includes("Antigravity")) {
               ...
               if (match) {
                 procs.push({ pid: parseInt(match[1]), name: "Antigravity", cmd: line });
               }
             }
          } else {
             ...
             if (rest.includes("Antigravity") || rest.includes("antigravity")) {
               procs.push({ pid, name: parts[1], cmd: rest });
             }
          }
        }
        return procs;
    ...
    const targetProcs = getProcesses().filter(p => {
      // Exclude self
      if (p.pid === currentPid) return false;
      // Exclude this electron app (if named Antigravity Manager)
      if (p.cmd.includes("Antigravity Manager") || p.cmd.includes("electron-shadcn")) return false;

      // Match Antigravity
      if (platform === "win32") {
        return p.cmd.includes("Antigravity.exe") || (p.cmd.includes("antigravity") && !p.cmd.includes("manager"));
      } else {
    ...
    logger.info(`Found ${targetProcs.length} remaining Antigravity processes. Killing...`);
...
// Helper to open URI protocol (matching Python implementation)
...
export async function startAntigravity(useUri = true): Promise<void> {
  ...
  // Try URI protocol first (more reliable, matching Python)
  if (useUri) {
